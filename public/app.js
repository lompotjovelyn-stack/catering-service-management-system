...
async function renderBookingForm() {
    ...
    const menuOptions = menuItems.map((item) => `
        <option value="${escapeHtml(item.id)}" data-price="${escapeHtml(item.pricePerGuest)}" data-name="${escapeHtml(item.name)}">
            ${escapeHtml(item.category)} - ${escapeHtml(item.name)} (${escapeHtml(item.pricePerGuest)} per guest)
        </option>
    `).join('');
...
<<<<<<< HEAD
                <div class="col-md-8">
                    ...
                    <input type="hidden" name="packageName" id="packageNameInput">
                </div>
                <div class="col-md-4">
        ...
        <form id="vendorForm" class="work-panel mb-3">
            <div class="row g-3">
                ...
                <div class="col-md-8">
                    ...
                    <input class="form-control" name="suppliedItems">
                </div>
                <div class="col-12">
...
function updateBookingEstimate() {
    ...
    if (packageNameInput) {
        packageNameInput.value = selectedOption?.dataset.name || '';
    }

    if (estimatedTotal) {
        estimatedTotal.value = (price * guests).toFixed(2);
    }
}
